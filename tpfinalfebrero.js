...
function preload() {
  ...
  for (let i = 0; i < 3; i++) {
    imgVidas.push(loadImage('data/vida.png'));
  }
}
...
class Personaje {
  ...
  mostrar() {
    image(imgPersonaje, this.x, this.y, tamCelda, tamCelda);
     ...
     for (let i = 0; i < imgVidas.length; i++) {
    image(imgVidas[i], 480 + i * 40, 10, 30, 30);
  }
...
function mostrarPantallaInicio() {
  ...
  fill(0);
  text("Jugar", width / 2, height / 2 + 7);

  fill(100, 255, 200);
  rect(width / 4, height / 2 + 50, width / 2, 50);
  fill(0);
  text("Créditos", width / 2, height / 2 + 87);
}
...
function mostrarPantallaInstrucciones() {
  ...
  fill(0);
  text("Continuar", width / 2, height / 2 + 37);
}
...
function ejecutarJuego() {
  if (ganaste || perdiste) {
    ...
    rect(width / 4, height / 2 + 50, width / 2, 50);
    fill(0);
    text("Volver al inicio", width / 2, height / 2 + 83);
    noLoop();
    return;
  }
